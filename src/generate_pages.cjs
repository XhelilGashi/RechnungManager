  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <button 
          onClick={() => showToast('In Kürze verfügbar', 'success')}
          className="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium text-sm shadow-sm"
        >
          Neu anlegen
        </button>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-10 text-center flex flex-col items-center justify-center min-h-[500px]">
        <div className="w-20 h-20 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mb-6">
          <${p.icon} className="w-10 h-10" />
        </div>
        <h3 className="text-xl font-bold text-slate-800 mb-3">${p.title} Modul geladen</h3>
        <p className="text-slate-500 max-w-md text-base leading-relaxed">
          Dieses Modul arbeitet nun völlig unabhängig mit eigenen States und Daten.<br/>
          <em>(Die spezifische Ansicht und Logik wird in Kürze freigeschaltet)</em>
        </p>
      </div>
    </div>
  );
